...
class VendorKyc extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function updateStatusByVendorId($vendorId, $status, $kycUserId=null) {
        ...
        $toBeUpdated = ['kyc_status' => $status];

        if ($kycUserId !== null) {
            $toBeUpdated['kyc_user_id'] = $kycUserId;
        }

        $conn->update($table, $toBeUpdated, ['vendor_id=?' => $vendorId]);
    }
}
