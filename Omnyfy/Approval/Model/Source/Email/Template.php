...
class Template extends \Magento\Framework\DataObject implements \Magento\Framework\Option\ArrayInterface
{
    ...
    public function toOptionArray()
    {
        ...
        if (!($collection = $this->_coreRegistry->registry('config_system_email_template'.static::EMAIL_TEMPLATE))) {
            $collection = $this->_templatesFactory->create()
                ->addFieldToFilter('orig_template_code', static::EMAIL_TEMPLATE);
            $collection->load();
            $this->_coreRegistry->register('config_system_email_template'.static::EMAIL_TEMPLATE, $collection);
        }
