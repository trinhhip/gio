...
class MassDeclineProduct extends Action
{
    /**
     * @var Filter
     */
                ...
                foreach ($collection->getItems() as $item) {
                    ...
                    if (!empty($approvalProduct = $approvalProductModel->getCollection()->addFieldToFilter('product_id', $item->getId())->getItems())) {
                        foreach ($approvalProduct as $record) {
                            ...
                            $approvalProductModel->setData('status', \Omnyfy\Approval\Model\Source\Status::STATUS_REVIEW_FAILED);
                            $item->setData('approval_status', \Omnyfy\Approval\Model\Source\Status::STATUS_REVIEW_FAILED);
                            $item->save();
                            $historyModel->save();
