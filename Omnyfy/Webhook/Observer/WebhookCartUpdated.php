...
class WebhookCartUpdated implements ObserverInterface
{
    protected $dateTime;
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($this->helper->isEnable($storeId)) {
            ...
            $type = $this->helper->getWebhookTypeIdByType('cart.updated');
            ...
            if ($this->helper->isEnableSchedule($storeId)) {
                ...
                if (count($webhooks)) {
                    try {
                        ...
                        $this->logger->critical(__("Error on cart.updated schedule: %1", $e->getMessage()));
                    }
                }
                ...
                if (count($webhooks)) {
                    try {
                        ...
                        $this->logger->critical(__("Error on cart.updated webhook: %1", $e->getMessage()));
                    }
                }
            }
        }
