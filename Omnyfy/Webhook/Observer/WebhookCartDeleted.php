...
class WebhookCartDeleted implements ObserverInterface
{
    protected $dateTime;
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($this->helper->isEnable($storeId)) {
            ...
            $type = $this->helper->getWebhookTypeIdByType('cart.deleted');
            ...
            if ($this->helper->isEnableSchedule($storeId)) {
                ...
                if (count($webhooks)) {
                    try {
                        ...
                        $this->logger->critical(__("Error on cart.deleted schedule: %1", $e->getMessage()));
                    }
                }
                ...
                if (count($webhooks)) {
                    try {
                        ...
                        $this->logger->critical(__("Error on cart.deleted webhook: %1", $e->getMessage()));
                    }
                }
            }
        }
