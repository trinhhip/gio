 ...
 */
namespace Omnyfy\VendorAuth\Model\Oauth;
...
class Token extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $this->_init(\Omnyfy\VendorAuth\Model\ResourceModel\Oauth\Token::class);
    }
    ...
    {
        $integrationCollection = $this->_integration->loadByConsumerId($consumerId);
        $vendorId = '';
        if($integrationCollection){
           if ($integrationCollection->getVendorId()) {
               $vendorId = $integrationCollection->getVendorId();
           } 
        }
