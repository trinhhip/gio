...
class WholeSalePayout extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                if ($this->isWholeSaleVendor()) {
                    $payoutAmountWholesaleVendor = $this->getWholeSalePayoutTotal($item['order_id'], $item['vendor_id'], $item['id']);
    ...
    public function getWholeSalePayoutTotal($orderId, $vendorId, $mcmOrderId){
        $total = $this->vendorPayout->getPayoutTotalWholesaleVendor($orderId, $vendorId, $mcmOrderId);
        if(!empty($total)){
