<?php
namespace Omnyfy\Vendor\Ui\DataProvider\Source\Form;
...
class SourceDataProvider extends \Magento\InventoryAdminUi\Ui\DataProvider\SourceDataProvider
{
    ...
    protected function searchResultToOutput(SearchResultInterface $searchResult)
    {
        ...
        foreach ($searchResult->getItems() as $item) {
            ...
            $vendorId = $this->collectionFactory->create()->getItemByColumnValue('source_code', $sourceCode)->getVendorId();
        ...
        if (!empty($vendorInfo)) {
            ...
            foreach ($arrItems['items'] as $item) {
                if ($item['vendor_id'] == $vendorId) {
                    array_push($arrDataByVendor, $item);
                }
            }
            $arrItems['items'] = $arrDataByVendor;
            $arrItems['totalRecords'] = count($arrDataByVendor);
    ...
    private function getStockIdsBySource($sourceCode) {
