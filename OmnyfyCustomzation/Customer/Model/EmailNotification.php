...
namespace OmnyfyCustomzation\Customer\Model;
...
class EmailNotification implements EmailNotificationInterface
{
    ...
    public function credentialsChanged(
        ...
        if ($origCustomerEmail != $savedCustomer->getEmail()) {
            ...
            $emails = [
                'old' => $origCustomerEmail,
                'new' => $savedCustomer->getEmail()
            ];
            $this->emailChanged($savedCustomer, $origCustomerEmail, $emails);
            $this->emailChanged($savedCustomer, $savedCustomer->getEmail(), $emails);
            return;
        }
     ...
     * @return void
     */
    private function emailChanged(CustomerInterface $customer, $email, $emails = [])
    {
        $storeId = $customer->getStoreId();
        if (!$storeId) {
            $storeId = $this->getWebsiteStoreId($customer);
        ...
        $this->sendEmailTemplate(
            ...
            [
                ...
                'original_email' => $emails['old'],
                'changed_email' => $emails['new'],
            ],
            $storeId,
            $email
        );
